...
func (g *Generator) ParserAST(p *Parser, structs []string, prefix string) (ret *Generator) {
	...
	//g.config.PrimaryIdName = g.structConfigs[0].OptionFields[0].FieldName

	for _, one := range g.structConfigs {
		if !strings.HasSuffix(one.StructName, "RepoQueryBuilder") {
			g.config.PrimaryIdName = one.OptionFields[0].FieldName
			break
		}
	}
