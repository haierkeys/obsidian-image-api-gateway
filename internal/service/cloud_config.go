...
type CloudConfig struct {
	...
	UpdatedAt       timef.Time `json:"updatedAt"`       //
	CreatedAt       timef.Time `json:"createdAt"`
}
...
func (svc *Service) CloudConfigList(uid int64, pager *app.Pager) ([]*CloudConfig, int, error) {
	...
	// 获取指定用户的云存储配置列表
	daoList, err := svc.dao.GetListByUid(uid, pager.Page, pager.PageSize)
	if err != nil {
		return nil, 0, err // 如果发生错误，返回 nil 和错误信息
	}
...
func (svc *Service) CloudConfigUpdateAndCreate(uid int64, params *CloudConfigRequest) error {
	...
	if params.Id == 0 {
		_, err := svc.dao.Create(da, uid)
		if err != nil {
			// 如果发生错误，返回错误信息
			return err
		}
	} else {
		err := svc.dao.Update(da, uid, params.Id)
		if err != nil {
			// 如果发生错误，返回错误信息
			return err
		}
